...
- Browser から Windows への upload は、ディスクへ確定済みの offset から再開できます。
...
1. [v2.0.0-rc.2 Release](https://github.com/SpecQR/LocalFileTransfer/releases/tag/v2.0.0-rc.2) を開きます。
2. 通常の Intel/AMD Windows PC では `x64-Portable.exe`、Windows on ARM では `arm64-Portable.exe` を選びます。
3. `SHA256SUMS.txt` と download したファイルの SHA-256 が一致することを確認します。
...
RC.2 の実行ファイルは Authenticode 未署名です。Windows SmartScreen が警告を表示する場合があります。
...
この Release では、86 件の unit/integration test と 2 件の end-to-end scenario、合計 88 scenario が通過しています。x64 packaged application は launch、service fault recovery、clean shutdown、DPI geometry、security fuse、audit、artifact hash の検証を通過しました。

物理 iPhone Safari、Android Chrome、Windows on ARM は、引き続き release qualification の manual gate です。未実施の検証を実施済みとは記載しません。
