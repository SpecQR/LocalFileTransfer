...
- 物理 iPhone SE（第2世代）の Safari で、共有テキスト入力欄を選択してもページが自動拡大されないことを確認しました。
- 物理 iPhone 16e の Safari でも、同じフォーカス挙動を確認しました。

この確認は共有テキスト入力時の自動ズーム修正を対象とします。物理 Android、Windows on ARM など、実施していない qualification item を検証済みとは表記しません。
