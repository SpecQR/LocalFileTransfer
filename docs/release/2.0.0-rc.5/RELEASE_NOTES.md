# Local File Transfer 2.0.0-rc.5
...
RC.5 の app 機能は RC.3 candidate と同じです。RC.3 は Windows Server 2025 hosted runner の firewall 非対話待機、RC.4 は service recovery 成功後の PowerShell `Get-FileHash` 不在により、どちらも公開前 gate で停止しました。GitHub Release は作成せず、両 tag は履歴として保持しています。

RC.5 は release qualification を Windows Server 2022 に固定したまま、artifact SHA-256 と Authenticode certificate-table presence を Node.js で検査します。PowerShell module の暗黙読み込みに依存しません。
...
- Utility Process 復旧後も同じ `localUrl` なら BrowserWindow を再読み込みせず、既存 UI state と SSE/poll reconnect を維持します。Port が変わった場合だけ新しい `/app` を load します。
...
- Packaged smoke と release evidence は同じ Node/PE inspector を使用し、truncated/invalid PE と certificate table 境界を fail closed で検出します。
...
- `Local.File.Transfer-2.0.0-rc.5-x64-Portable.exe`: 通常の Intel/AMD Windows
- `Local.File.Transfer-2.0.0-rc.5-arm64-Portable.exe`: Windows on ARM
...
```powershell
Get-FileHash -Algorithm SHA256 -LiteralPath .\Local.File.Transfer-2.0.0-rc.5-x64-Portable.exe
gh attestation verify .\Local.File.Transfer-2.0.0-rc.5-x64-Portable.exe --repo SpecQR/LocalFileTransfer
```
