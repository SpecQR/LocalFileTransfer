# Local File Transfer 2.0.0-rc.3
...
- `Local.File.Transfer-2.0.0-rc.3-x64-Portable.exe`: 通常の Intel/AMD Windows
- `Local.File.Transfer-2.0.0-rc.3-arm64-Portable.exe`: Windows on ARM
...
x64 packaged smoke は HTTP、SQLite、utility-process restart、graceful shutdown を分離して検証するため `--disable-gpu` で起動します。QR と renderer の描画は別の Chromium/WebKit E2E と 100%/125%/150%/200% DPI geometry gate で検証します。
