...
RC.4 が Authenticode 未署名なら、Release Notes と evidence に `NotSigned` を残します。Publisher identity があるように見せてはいけません。
...
2. `main` の reviewed commit に annotated tag `v2.0.0-rc.4` を付ける。
3. Tag を push する。
