...
RC.4 の public upload surface は `HEAD` と `PATCH` だけです。旧 `PUT .../chunks` endpoint は current client から参照されず、default runtime でも register しません。`/api/local/*` も test fixture が `enableLegacyRoutes` を明示した場合だけ有効です。
