...
`local/` implementation と regression test は legacy behavior の監査用に残っていますが、RC.5 の default runtime は register しません。`buildApp({ enableLegacyRoutes: true })` は test fixture 専用 opt-in です。旧 `PUT .../chunks` public route は削除済みで、current upload は `HEAD` + `PATCH` だけです。

### `apps/web`

- `TransferRoomPage.tsx`: RC.5 の canonical room UI、queue orchestration、failure-only reconnect state
...
- `release-assets-lib.mjs`: artifact naming、SHA-256、PE security directory / `WIN_CERTIFICATE` structural inspection、manifest、provenance の共通 pure functions
- `inspect-release-artifact.mjs`: 1 Portable EXE を同じ共通関数で検査する JSON CLI。Packaged smoke が使用する
- `generate-release-evidence.mjs`: 両 architecture の artifact、4 dependency scope、SBOM、audit、license inventory をまとめる
- `test-packaged-windows.ps1`: x64 Portable の service interruption/recovery、SQLite、HTTP、shutdown を隔離検証する。Hash/PE 判定は Node CLI へ委譲し、PowerShell module 自動読み込みに依存しない
...
- Version/tag: `2.0.0-rc.5` / `v2.0.0-rc.5`
...
Exact automated count と artifact hash は RC.5 release evidence から読むこと。未実施の check を positive claim に変更せず、Artifact が変わった場合は hash、evidence、attestation を再生成します。
